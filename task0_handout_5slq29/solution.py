...
def log_posterior_probs(x):
    ...
    Returns:
        log_p (np.ndarray): a numpy array of size 3, containing the Bayesian log-posterior probabilities
    ...
    for i in range(3):
        temp[i] = np.log(PRIOR_PROBS[i]) + np.sum( np.log(HYPOTHESIS_SPACE[i].pdf(x)) )

    log_p = temp - logsumexp(temp)
